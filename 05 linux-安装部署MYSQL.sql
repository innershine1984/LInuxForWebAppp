#本文档为centos6.5系统下安装配置mysql5.6的指导手册

#--centos系统下安装mysql有三种方式：
# ① 二进制 rpm 安装 mysql
# ② 二进制免编译安装 mysql
# ③ 源码包安装 mysql

#本手册只针对已预编译的二进制文件安装mysql数据库操作

--1.卸载CentOS自带数据库
-- ① 查询已安装的mariadb
[root@cjolie /]# rpm -qa|grep mariadb

-- ② 卸载数据库
[root@cjolie /]# rpm -e --nodeps mariadb-libs

--2.安装依赖
[root@cjolie /]# yum -y install autoconf
[root@cjolie /]# yum provides libnuma.so.1

--3.安装mysql5.6
----准备工作
-- ① 创建mysql用户
[root@cjolie /]# useradd -s /sbin/nologin -M mysql

-- ② 上传安装包到指定安装目录下，并解压文件
 
[root@jolie local]# tar -zxvf mysql-5.6.39-linux-glibc2.12-x86_64.tar.gz

-- ③重命名为mysql
[root@jolie local]# mv mysql-5.6.39-linux-glibc2.12-x86_64 mysql

-----文件配置
-- ① my.cnf文件复制
[root@jolie local]# cp /usr/local/mysql/support-files/my-default.cnf /etc/my.cnf

-- ② 编辑my.cnf 
#二进制安装，默认配置文件在/etc/my.cnf
[root@jolie local]# vi /etc/my.cnf

-- ③ 清空my.cnf内容，将以下内容粘贴进去，并保存
...
--4.将mysql安装文件授权给mysql用户
[root@jolie /]# chown -R mysql.mysql /usr/local/mysql

--5.初始化数据库
[root@jolie /]# /usr/local/mysql/scripts/mysql_install_db  --defaults-file=/etc/my.cnf  --user=mysql  --basedir=/usr/local/mysql  --datadir=/usr/local/mysql/data

--6.配置mysql服务
-- ① 复制服务文件
[root@jolie /]# cp /usr/local/mysql/support-files/mysql.server /etc/init.d/mysqld

-- ② 服务文件授权
[root@jolie /]# chmod 755 /etc/init.d/mysqld

-- ③ 将服务文件中默认路径替换为安装路径 
#mysql二进制安装默认的路径为/usr/local/mysql,启动脚本里/usr/local/mysql需要替换
[root@jolie /]# sed -i 's#/usr/local/mysql#/opt/mysql#g' /usr/local/mysql/bin/mysqld_safe /etc/init.d/mysqld

-- ④ 启动mysql
[root@jolie /]# service mysqld start


--7.将mysql服务添加到系统服务，并设置开机启动
[root@jolie /]# chkconfig --add mysqld
[root@jolie /]# chkconfig mysqld on
[root@jolie /]# chkconfig --list mysqld

--7.配置环境变量
[root@jolie /]# echo 'export PATH=/usr/local/mysql/bin:$PATH' >> /etc/profile
[root@jolie /]# source /etc/profile

--8.设置mysql root 密码
[root@jolie /]#  mysqladmin -u root password 'mysql19840320'

--9.允许本机和任何地址远程访问
[root@jolie /]#  mysql -u root -p
Enter password:
 
mysql> GRANT ALL privileges ON *.* TO 'root'@'localhost' IDENTIFIED BY 'mysql19840320' WITH GRANT OPTION;
mysql> GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY 'mysql19840320' WITH GRANT OPTION;
mysql> flush privileges;

--10.配置防火墙策略
-- ① 将mysql服务访问端口添加到防火墙配策略中（只适用于centos7以下版本系统）

[root@jolie /]# vi /etc/sysconfig/iptables
-A INPUT -m state --state NEW -m tcp -p tcp --dport 3306 -j ACCEPT

-- ② 重启防火墙服务
[root@jolie /]# service iptables restart
